...
class JsonPathReader extends JsonPathUtility {
  static final JsonPathReader _instance = JsonPathReader._internal();
  ...
  dynamic read(String json, String path, {String? fallback}) {
    ...
    if (decodedJson is LinkedHashMap<String, dynamic>) {
      ...
      return _getSearchKeyFromMap(decodedJson as LinkedHashMap<String, dynamic>, key, splitPath);
    } else {
      return null;
    }
  }
    ...
    while (pos < keys.length && keys.isNotEmpty) {
      ...
      if (keyName == nodeName || keyName == nodeName.split("[")[0]) {
        ...
        if (splitPath.isEmpty) {
          ...
          if (keyValue is LinkedHashMap<String, dynamic>) {
            var key = splitPath.elementAt(0);
            splitPath.removeAt(0);
  ...
  Object? _getSearchKeyFromList(
    ...
    if (splitPath.isEmpty) {
      ...
      if (data is LinkedHashMap<String, dynamic>) {
        var key = splitPath.elementAt(0);
        splitPath.removeAt(0);
